...
if (isset($_POST['cancel_order'])) {
    ...
    $deleteStmt = $conn->prepare("DELETE FROM netflix_orders WHERE ordernum = ? AND username = ?");
    $deleteStmt->bind_param("is", $ordernum, $sessionUsername); // Use $sessionUsername instead of $username
    $deleteStmt->execute();
    $deleteStmt->close();
}

// Fetch orders
$stmt = $conn->prepare("SELECT ordernum, service, screenOption, durationOption, email, contactNumber, message, price FROM netflix_orders WHERE username = ?");
$stmt->bind_param("s", $sessionUsername); // Use $sessionUsername instead of $username
$stmt->execute();
